...
class MemoryGame(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        self.start_button.move(185, 400)
        ...
        if len(self.player_sequence) == len(self.sequence):
            if self.player_sequence == self.sequence:
                QTimer.singleShot(0, lambda: [button.setStyleSheet("background-color: lime") for button in
                                              self.buttons])
                QTimer.singleShot(305, lambda: [button.setStyleSheet("background-color: lime") for button in
                                                self.buttons])
                self.label2.show()
                phrases = [
                ...
                QTimer.singleShot(0,lambda: [button.setStyleSheet("background-color: red") for button in
                                               self.buttons])
                QTimer.singleShot(305, lambda: [button.setStyleSheet("background-color: red") for button in
                                                self.buttons])
